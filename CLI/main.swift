...
let usage = """
Usage: jsonav [--test] [--timeout SECONDS] [--json TEXT | --file PATH]
Without --json/--file, read UTF-8 text from standard input (maximum 10 MiB).
Default target: JSONav Personal; --test selects JSONav Personal Test.
...
do {
    ...
    let preferred = useTest
        ? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Applications/JSONav Personal Test.app")
        : URL(fileURLWithPath: "/Applications/JSONav Personal.app")
    let appURL: URL
    if FileManager.default.fileExists(atPath: preferred.path) {
        appURL = preferred
    } else if let located = NSWorkspace.shared.urlForApplication(withBundleIdentifier: identifier) {
        appURL = located
    } else { throw ImportError("Install \(useTest ? "JSONav Personal Test" : "JSONav Personal") first") }
    ...
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("jsonav-\(request.id.uuidString)", isDirectory: true)
